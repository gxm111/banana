...
_RimPower("Rim Position", range(0,4)) = 2 _RimStrength("Rim Strength", range(0,1)) = 1
}
SubShader {
Tags { "RenderType"="Opaque" } LOD 200
CGPROGRAM
#pragma surface surf Cartoon
sampler2D _MainTex; sampler2D _TransitionTexture; half4 _BrightColor;
half4 _DarkColor;
half _Threshold1;
half _Threshold2;
...
sampler2D _BrightTexture; sampler2D _DarkTexture;
...
void surf (Input IN, inout SurfaceOutputCustom o) { half4 c = tex2D (_MainTex, IN.uv_MainTex); o.UV = IN.uv_MainTex;
half NdotView = 1 - dot(normalize(IN.viewDir), o.Normal); NdotView = pow(NdotView, _RimPower);
o.Emission = NdotView * _RimColor * _RimStrength;
o.Albedo = c * ( 1 - NdotView * _RimStrength); o.Alpha = c.a;
}
ENDCG
}
FallBack "Diffuse" 
